...
SMALL_GEMINI_MODEL = 'gemini-2.5-flash-lite-preview-06-17'
...
def get_api_key_env_var(model_name: str) -> str:
    """Get the appropriate API key environment variable name based on model."""
...
class GraphitiLLMConfig(BaseModel):
    ...
    def from_env(cls) -> 'GraphitiLLMConfig':
        ...
        if azure_openai_endpoint is None:
            ...
            api_key_env_var = get_api_key_env_var(model)
            api_key = os.environ.get(api_key_env_var)

            # Set appropriate defaults for Gemini models
            if is_gemini_model(model):
                if model_env == '':
                    model = DEFAULT_GEMINI_MODEL
                if small_model_env == '':
                    small_model = SMALL_GEMINI_MODEL
